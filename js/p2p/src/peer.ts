...
import { type Libp2p, type Stream, type Connection } from '@libp2p/interface'
import { multiaddr } from '@multiformats/multiaddr';
import {
  privateKeyFromProtobuf, // For protobuf-encoded keys
 ...
 */
export class Peer implements Host {
  private helia: Helia;
  private pubsubEventListeners = new Map<string, (evt: any) => void>();
  private blockAccessFunc?: BlockAccessFunc;

  private constructor(helia: Helia) {
    this.helia = helia;
  }
  ...
  static async create(
    ...
    options?: {
      ...
      enableRelay?: boolean;
      client?: any; // DefraDB WASM client instance
    }
  ): Promise<Peer> {
  ...
  async close(): Promise<void> {
    ...
    const pubsub = (this.helia.libp2p.services.pubsub as GossipSub);
    if (pubsub) {
   ...
   */
  async send(data: Uint8Array, peerID: string, protocolID: string): Promise<void> {
  ...
  async sign(data: Uint8Array): Promise<Uint8Array> {
    ...
    const privateKey = (this.helia.libp2p as any).components?.privateKey;
    if (!privateKey) {
  ...
  setStreamHandler(protocolID: string, handler: StreamHandler): void {
    ...
    this.helia.libp2p.handle(protocolID, (stream: Stream, connection: Connection) => {
   ...
   */
  async addPubSubTopic(topicName: string, subscribe: boolean, handler: PubsubMessageHandler): Promise<void> {
    if (!topicName) {
   ...
   */
  async removePubSubTopic(topic: string): Promise<void> {
    if (!topic) {
      throw new Error('Topic name is required');
    }
   ...
   */
  async publishToTopicAsync(topic: string, data: Uint8Array): Promise<void> {
  ...
  async publishToTopic(
    ...
    const responseQueue: PubsubResponse[] = [];
    let isDone = false;
    let resolveNext: ((value: IteratorResult<PubsubResponse>) => void) | null = null;
    ...
    return {
      next: async (): Promise<IteratorResult<PubsubResponse>> => {
        ...
        return new Promise<IteratorResult<PubsubResponse>>((resolve) => {
          resolveNext = resolve;
        });
      },
