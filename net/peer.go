...
func (p *Peer) RegisterNewDocument(
	...
	// register topic
	if err := p.server.addPubSubTopic(docID.String(), !p.server.hasPubSubTopic(schemaID)); err != nil {
		log.ErrorE(
			p.ctx,
...
func (p *Peer) pushToReplicator(
	...
	docIDCh <-chan client.DocIDsResult,
	pid peer.ID,
) {
	for result := range docIDCh {
		if result.Err != nil {
			log.ErrorE(ctx, "Key channel error", result.Err)
			continue
		}
		dsKey := core.DataStoreKeyFromDocID(result.ID)
...
func (p *Peer) handleDocCreateLog(evt events.Update) error {
	...
	err = p.RegisterNewDocument(p.ctx, docID, evt.Cid, evt.Block, evt.SchemaID)
	if err != nil {
		return err
	}
