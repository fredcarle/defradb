...
func runCollectionBenchGet(
	...
	docIDs, err := benchutils.BackfillBenchmarkDB(
		b,
		ctx,
		collections,
		fixture,
	...
	// run benchmark
	if doSync {
		return runCollectionBenchGetSync(b, ctx, collections, fixture, docCount, opCount, docIDs)
	}
	return runCollectionBenchGetAsync(b, ctx, collections, fixture, docCount, opCount, docIDs)
}
	...
	for i := 0; i < b.N; i++ { // outer benchmark loop
		for j := 0; j < opCount/numTypes; j++ { // number of Get operations we want to execute
			for k := 0; k < numTypes; k++ { // apply op to all the related types
				collections[k].Get(ctx, docIDs[j][k], false) //nolint:errcheck
			}
		}
	}
	b.StopTimer()
