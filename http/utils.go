...
// mustGetContextClient returns the Client from the http request context or panics.
//
// This should only be called from functions within the http package.
func mustGetContextClient(req *http.Request) Client {
	return req.Context().Value(clientContextKey).(Client) //nolint:forcetypeassert
}
